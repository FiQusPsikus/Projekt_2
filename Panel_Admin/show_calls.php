        ...
        <?php
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo '<div style="border:1px solid black;width:80%;height:70px;margin:auto;line-height:70px;padding-left:50px;">'.$row['tresc'].'
                   
                    <a href="delete_call.php?id='.$row['id_zgloszenia'].'">
                        <button type="button" class="btn btn-danger float-right" style="margin-right:10px;margin-top:15px;">Usuń zgłoszenie</button>
                    </a> 
                    <a href="call_into.php?id='.$row['id_zgloszenia'].'">
                        <button type="button" class="btn btn-link">Czytaj dalej</button>
                    </a> 
                    </div><br>';
                }
            } else {
                echo "<div style='text-align:center;height:150px;line-height:170px;'>Brak zgłoszeń...</div>";
            }
            $conn->close();
            ?>
